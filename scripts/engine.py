...
def analyze_set50_index(stocks=None):
    """
    Analyze SET50 Index — try Yahoo Finance first, fallback to synthetic from constituents

    Returns: dict ของ SET50 index data + prediction, หรือ None ถ้าไม่ได้
    """
    # ลอง ticker หลายแบบ
    ...
    # ถ้า Yahoo ดึงไม่ได้ → ใช้ synthetic จาก 50 หุ้น
...
def synthesize_set50(stocks):
    """
    คำนวณ Synthetic SET50 จาก 50 หุ้น เมื่อ Yahoo ticker ไม่พร้อมใช้
    """
    if not stocks:
        return None
    ...
    # ประมาณราคา SET50 จาก swing data + apply daily change
    ...
    data = {
        ...
        'vol_trend': None,
        'atr': None, 'hist_vol': None, 'vol_class': 'UNKNOWN',
        'fibs': fibs, 'fib_pos': fib_pos,
        'swing_high': SWING_HIGH, 'swing_low': SWING_LOW,
